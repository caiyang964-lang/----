...
import { Upload, Image as ImageIcon, Settings, Trash2, Video, Server, Shield, LayoutDashboard, Database, RefreshCw, Plus, FileVideo, FileImage, Search, Bell, Copy, CheckCircle2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      ...
      const newItem: MediaItem = {
        ...
        url: isVideo ? 'https://www.w3schools.com/html/mov_bbb.mp4' : URL.createObjectURL(file), 
  ...
  return (
    <div className="flex h-screen bg-[#FDFDFD] flex-col md:flex-row font-sans text-gray-900 overflow-hidden font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* Sidebar */}
      <aside className="w-full md:w-[260px] bg-[#F7F7F8] border-r border-gray-200/60 flex flex-col shrink-0 overflow-visible z-20">
        ...
        <nav className="flex-1 px-4 space-y-1.5 flex flex-row md:flex-col overflow-x-auto md:overflow-visible">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#FDFDFD]">
        ...
        {/* Header */}
        <header className="h-16 border-b border-gray-200/60 flex items-center justify-between px-8 bg-white/50 backdrop-blur-sm z-10 shrink-0">
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 bg-gray-900 text-white hover:bg-gray-800 px-5 py-2.5 rounded-full font-medium transition-all active:scale-95 disabled:opacity-70 disabled:pointer-events-none shadow-md shadow-gray-900/10"
                    >
                      {isUploading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                      {isUploading ? '正在直传至 COS...' : '上传新资源'}
                    </button>
                  </div>
                    ...
                    <AnimatePresence>
                      {mediaItems.map((item, i) => (
                        <motion.div 
                          ...
                          className="bg-white rounded-2xl border border-gray-200/60 overflow-hidden shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_12px_24px_-8px_rgba(0,0,0,0.1)] hover:border-gray-300 transition-all group flex flex-col"
                        >
                          {/* Thumbnail Area */}
                          <div className="aspect-[4/3] bg-gray-100 relative overflow-hidden p-1">
                            <div className="w-full h-full relative rounded-xl overflow-hidden bg-gray-900 group-hover:shadow-inner transition-all">
                              ...
                              {/* Type Badge */}
                              <div className="absolute top-3 left-3 bg-black/50 backdrop-blur-md border border-white/10 text-white/90 font-mono text-[10px] px-2 py-0.5 rounded shadow-sm flex items-center gap-1.5 uppercase font-semibold tracking-wider z-10">
                                {item.type === 'video' ? <FileVideo className="w-3 h-3" /> : <FileImage className="w-3 h-3" />}
                                {item.type}
                              </div>
                              
                              {/* Overlay Actions */}
                              <div className="absolute inset-0 bg-gray-900/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-4 backdrop-blur-[2px]">
                                <button 
                  ...
                  {mediaItems.length === 0 && (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500 mb-6 max-w-sm mx-auto">您的 COS 存储桶中暂无数据。将文件直传至云端以在网站上进行托管分发。</p>
                      <button 
                        onClick={() => fileInputRef.current?.click()}
